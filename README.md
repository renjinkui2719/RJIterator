...
###### 因为rj_async仅负责调度提供的异步任务(包装在rj_yield内)且在调度时不阻塞主线程.这里async的含义并不是启动子线程来执行块，而是块内部以异步方式调度,这其中有很大区别，即：异步==不阻塞, 异步!=子线程, 如果rj_async仅仅是启动子线程来达到异步，那和直接dispatch_async没什么区别.
